...
export default function StoreLocatorPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8F5F0] dark:bg-stone-950">
      <Header />
      
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10 fade-in-up">
        <h1 className="text-3xl md:text-4xl font-bold text-center text-[#7B4B36] dark:text-[#E2C3A5] mb-10" style={{ fontFamily: 'serif' }}>
          Hệ thống cửa hàng
        </h1>

        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 h-auto lg:h-[600px]">
          {/* Left panel */}
          <div className="w-full lg:w-[400px] shrink-0 bg-white dark:bg-gray-900 rounded-2xl border border-amber-100 shadow-sm p-6 lg:p-8 flex flex-col items-start relative overflow-hidden">
            {/* Background Decor */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-amber-50 dark:bg-amber-900/20 rounded-bl-full -z-0 opacity-50 pointer-events-none" />
            
            <h2 className="text-2xl font-bold text-[#7B4B36] dark:text-[#E2C3A5] mb-8 relative z-10" style={{ fontFamily: 'serif' }}>Tìm cửa hàng</h2>
            
            <div className="space-y-6 flex-1 w-full relative z-10">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Chọn tỉnh thành</label>
                <select 
                  value={selectedCity}
                  onChange={handleCityChange}
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition shadow-sm bg-gray-50 dark:bg-gray-950 text-gray-800 dark:text-gray-200"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Chọn cửa hàng</label>
                <select 
              ...
              {selectedStore && (
                <div className="pt-6 mt-6 border-t border-amber-100 space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-gray-100 mb-0.5 text-sm uppercase tracking-wide">Địa chỉ</p>
                      <p className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed">{selectedStore.address}</p>
                    </div>
                  </div>
                  
                  <div className="flex items-start gap-3 mt-4">
                    <Phone className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                    <p className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed"><span className="font-semibold text-gray-900 dark:text-gray-100">Liên hệ:</span> {selectedStore.phone}</p>
                  </div>
                  
                  <div className="flex items-start gap-3 mt-4">
                    <Clock className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                    <div className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                      <span className="font-semibold text-gray-900 dark:text-gray-100 block mb-1">Giờ hoạt động:</span>
          ...
          {/* Right panel (Map) */}
          <div className="flex-1 bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm h-[450px] lg:h-full border border-gray-200  relative p-2">
            <div className="w-full h-full rounded-xl overflow-hidden relative bg-gray-50 dark:bg-gray-950">
              {selectedStore ? (
                <iframe 
                ...
                <div className="w-full h-full flex items-center justify-center text-gray-500 dark:text-gray-400 flex-col gap-4">
