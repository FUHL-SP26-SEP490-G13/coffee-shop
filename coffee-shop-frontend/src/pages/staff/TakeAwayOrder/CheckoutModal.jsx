...
import {
  X, Banknote, CreditCard, Tag, Loader2,
  CheckCircle2, ChevronRight,
} from 'lucide-react';

const fmt = (n) => Number(n).toLocaleString('vi-VN') + ' đ';

// Các mệnh giá gợi ý khi thu tiền mặt
 ...
 *   subtotal: number,
 *   onClose: Function,
 *   onConfirm: Function(paymentMethod, discountCode, receivedAmount),
 *   loading: boolean,
 * }} props
 */
export function CheckoutModal({ subtotal, onClose, onConfirm, loading }) {
  ...
  // Gợi ý mệnh giá: >= finalAmount, tối đa 4 gợi ý
  const suggestions = (() => {
    const base = [
      finalAmount, // đúng tiền
      ...CASH_SUGGESTIONS.filter((v) => v > finalAmount),
    ];
    // Thêm làm tròn lên hàng chục nghìn gần nhất
  ...
  const handleApplyDiscount = async () => {
    ...
    setDiscountError('');
    try {
      // Gọi API validate discount — import takeawayService nếu cần
      // const res = await takeawayService.validateDiscount(discountCode, subtotal);
      // const d = res.data?.data || res.data;
      // setDiscountAmount(d.discount_amount || 0);
      // setDiscountApplied(true);

      // Placeholder: giả sử giảm 10%
      const mockDiscount = Math.round(subtotal * 0.1);
      setDiscountAmount(mockDiscount);
      setDiscountApplied(true);
    } catch (e) {
      setDiscountError(e?.response?.data?.message || 'Mã giảm giá không hợp lệ');
      setDiscountAmount(0);
  ...
  const canConfirm = paymentMethod === 'payos'
    || (paymentMethod === 'cash' && (receivedAmt === 0 || receivedAmt >= finalAmount));
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100"
                  />
                </div>
                <button
                  onClick={handleApplyDiscount}
                  disabled={!discountCode.trim() || checkingDiscount}
                  className="px-4 py-2.5 rounded-xl bg-amber-500 text-white text-sm font-semibold hover:bg-amber-600 disabled:opacity-40 flex items-center gap-1.5"
                >
                  {checkingDiscount
            ...
            <div className="space-y-3 animate-in fade-in duration-150">
              <div>
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={receivedInput
                    ? Number(String(receivedInput).replace(/\D/g, '')).toLocaleString('vi-VN')
                    : ''}
                  onChange={(e) => {
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm font-semibold text-gray-800 focus:outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100"
                />
              </div>
              ...
              {receivedAmt > 0 && (
                <div className="flex justify-between items-center px-4 py-2.5 bg-blue-50 rounded-xl border border-blue-100">
                  <span className="text-sm text-blue-700">Tiền thừa trả khách</span>
