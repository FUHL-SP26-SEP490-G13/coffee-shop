jest.mock('../../src/services/StaffDBService');

const StaffDBController = require('../../src/controllers/StaffDBController');
const dep1 = require('../../src/services/StaffDBService');

describe('StaffDBController', () => {
  const makeReq = () => ({
  ...
  it('StaffDBController - getOverview - TC-01: should handle success path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    let thrown = null;
    try {
      if (typeof StaffDBController.getOverview === 'function') {
        await StaffDBController.getOverview(req, res, next);
      }
    } catch (error) {
      thrown = error;
    }
    ...
    const reality = {
      hasMethod: typeof StaffDBController.getOverview === 'function',
    ...
    logCase({
      title: 'StaffDBController - getOverview - TC-01',
      input: { method: 'getOverview', req },
      expected: { type: 'success' },
      reality,
    });

    expect(typeof StaffDBController.getOverview).toBe('function');
  });

  it('StaffDBController - getOverview - TC-02: should handle 404-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    logCase({
      title: 'StaffDBController - getOverview - TC-02',
      input: { method: 'getOverview', req },
  ...
  it('StaffDBController - getOverview - TC-03: should handle 500-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    logCase({
      title: 'StaffDBController - getOverview - TC-03',
      input: { method: 'getOverview', req },
