...
jest.mock('../../src/services/NewsAIService');

const NewsAIController = require('../../src/controllers/NewsAIController');
const response = require('../../src/utils/response');
const dep1 = require('../../src/services/NewsAIService');


const { logTestCase } = require('../utils/logger');
describe('NewsAIController', () => {
  const makeReq = () => ({
  ...
  it('NewsAIController - suggestByTitle - TC-01: should handle success path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    let thrown = null;
    try {
      if (typeof NewsAIController.suggestByTitle === 'function') {
        await NewsAIController.suggestByTitle(req, res, next);
      }
    } catch (error) {
      thrown = error;
    }
    ...
    const reality = {
      hasMethod: typeof NewsAIController.suggestByTitle === 'function',
    ...
    logCase({
      title: 'NewsAIController - suggestByTitle - TC-01',
      input: { method: 'suggestByTitle', req },
      expected: { type: 'success' },
      reality,
    });

    expect(typeof NewsAIController.suggestByTitle).toBe('function');
  });

  it('NewsAIController - suggestByTitle - TC-02: should handle 404-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    logCase({
      title: 'NewsAIController - suggestByTitle - TC-02',
      input: { method: 'suggestByTitle', req },
  ...
  it('NewsAIController - suggestByTitle - TC-03: should handle 500-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    logCase({
      title: 'NewsAIController - suggestByTitle - TC-03',
      input: { method: 'suggestByTitle', req },
  ...
  it('NewsAIController - suggestContentBySummary - TC-04: should handle success path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    let thrown = null;
    try {
      if (typeof NewsAIController.suggestContentBySummary === 'function') {
        await NewsAIController.suggestContentBySummary(req, res, next);
      }
    } catch (error) {
      thrown = error;
    }
    ...
    const reality = {
      hasMethod: typeof NewsAIController.suggestContentBySummary === 'function',
      nextCalls: next.mock.calls.length,
      statusCalls: res.status.mock.calls.length,
      jsonCalls: res.json.mock.calls.length,
      uncaughtError: thrown ? thrown.message : null,
    ...
    logCase({
      title: 'NewsAIController - suggestContentBySummary - TC-04',
      input: { method: 'suggestContentBySummary', req },
      expected: { type: 'success' },
      reality,
    });

    expect(typeof NewsAIController.suggestContentBySummary).toBe('function');
  });

  it('NewsAIController - suggestContentBySummary - TC-05: should handle 404-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    const error404 = Object.assign(new Error("Not Found"), { statusCode: 404 });
    ...
    logCase({
      title: 'NewsAIController - suggestContentBySummary - TC-05',
      input: { method: 'suggestContentBySummary', req },
      expected: { type: 'error', statusCode: 404 },
      reality,
    });
  ...
  it('NewsAIController - suggestContentBySummary - TC-06: should handle 500-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    const error500 = Object.assign(new Error("Internal Server Error"), { statusCode: 500 });
    ...
    logCase({
      title: 'NewsAIController - suggestContentBySummary - TC-06',
      input: { method: 'suggestContentBySummary', req },
      expected: { type: 'error', statusCode: 500 },
      reality,
    });
