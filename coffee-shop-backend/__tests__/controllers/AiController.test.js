...
const dep1 = require('../../src/services/AiService');
...
describe('AiController', () => {
  ...
  it('AiController - chat - TC-01: should handle success path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    let thrown = null;
    try {
      if (typeof AiController.chat === 'function') {
    ...
    const reality = {
      hasMethod: typeof AiController.chat === 'function',
    ...
    logCase({
      title: 'AiController - chat - TC-01',
      input: { method: 'chat', req },
      expected: { type: 'success' },
      reality,
    });

    expect(typeof AiController.chat).toBe('function');
  });

  it('AiController - chat - TC-02: should handle 404-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
  ...
  it('AiController - chat - TC-03: should handle 500-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
