jest.mock('../../src/services/QrOrderService');
jest.mock('../../src/services/NotificationService');

const QrOrderController = require('../../src/controllers/QrOrderController');
const dep1 = require('../../src/services/QrOrderService');
...
describe('QrOrderController', () => {
  ...
  it('QrOrderController - checkout - TC-01: should handle success path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    let thrown = null;
    try {
      if (typeof QrOrderController.checkout === 'function') {
        await QrOrderController.checkout(req, res, next);
      }
    } catch (error) {
      thrown = error;
    }
    ...
    const reality = {
      hasMethod: typeof QrOrderController.checkout === 'function',
    ...
    logCase({
      title: 'QrOrderController - checkout - TC-01',
      input: { method: 'checkout', req },
      expected: { type: 'success' },
      reality,
    });

    expect(typeof QrOrderController.checkout).toBe('function');
  });

  it('QrOrderController - checkout - TC-02: should handle 404-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    logCase({
      title: 'QrOrderController - checkout - TC-02',
      input: { method: 'checkout', req },
  ...
  it('QrOrderController - checkout - TC-03: should handle 500-like error path', async () => {
    const req = makeReq();
    const res = makeRes();
    const next = jest.fn();
    ...
    logCase({
      title: 'QrOrderController - checkout - TC-03',
      input: { method: 'checkout', req },
